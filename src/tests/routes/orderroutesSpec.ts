...
describe('product route', () => {
    ...
    beforeAll( async()=> {
        ...
        .send({name: product.name, price: product.price, category: product.category}).set('Authorization', 'Bearer ' + token)).body
    ...
    it('create method creates a new order', async() => {
        ...
        .send({id: user.id, status: 'active'}).set('Authorization', 'Bearer ' + token)
        expect(result.status).toBe(200)
    })
    
    it('addProduct route adds a new product to an active order', async() => {
        const result = await request.post('/order/product')
        .send({id: user.id, quantity: 20, productid: product.id}).set('Authorization', 'Bearer ' + token)
        expect(result.status).toBe(200)
    })
    
    it('show method returns all products in the active order of the given user.', async() => {
        const result = await request.get('/order/' + user.id).set('Authorization', 'Bearer ' + token)
        expect(result.status).toBe(200)
    })
})