...
export class OrderStore {
  ...
  async show(id: string): Promise<OrderProduct[]> {
    try {
        ...
        const order: Order = (await conn.query(sql, [(id as unknown) as number, 'active'])).rows[0]
        if(order == null){
          return []
        }
  ...
  async addProduct(userid: number, quantity: number, productid: number): Promise<OrderProduct> {
    try {
        ...
        const result: OrderProduct = (await conn.query(sql, [quantity, orderid, productid])).rows[0]
